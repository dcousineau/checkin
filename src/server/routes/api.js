...
api.get("/stats", (req, res) => {
  Promise.all([
    ...
    let stats = allTix.map(ticket => ticket).reduce((stats = {}, ticket) => {
...
api.get("/tickets", (req, res) => {
  will(db.find.bind(db), {}).then(tickets => {
    tickets.sort((a, b) => {
        ...
        Promise.all(
          tickets.map(ticket =>
            will(
              db.update.bind(db),
              { id: ticket.id },
              { $set: omit(ticket, ["checkedIn"]) },
              { upsert: true }
            )
          )
        )
      ) //Update everything but preserve checkedIn
...
const printBadge = (badge, copies = 2) => {
  return new Promise((resolve, reject) => {
    ...
    const print = spawn("lp", [
      "-d",
      "Brother_QL_700",
      "-o",
      "media=Custom.200x100px",
      "-o",
      "Collate=True",
      "-n",
      `${copies}`
...
api.get("/report/checked-in", (req, res) => {
  ...
  will(db.find.bind(db), { checkedIn: true })
    ...
    .then(attendees =>
      [["ID", "Last Name", "First Name", "Company", "Type"]].concat(attendees)
    )
