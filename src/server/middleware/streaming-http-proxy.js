...
export default function streamingHttpProxy(host, _options = {}) {
    ...
    return (req, res, next) => {
        ...
        const externalRequest = (ishttps ? https : http).request(reqOptions, (response) => {
        ...
        externalRequest.on('socket', socket => {
            options.timeout && socket.setTimeout(options.timeout, () => externalRequest.abort());
        });

        externalRequest.on('error', err => {
            if (err.code === 'ECONNRESET') {
                res.setHeader('X-Timeout-Reason', `Timed out proxy request to ${host}${forwardPath} after ${options.timeout}ms.`);
